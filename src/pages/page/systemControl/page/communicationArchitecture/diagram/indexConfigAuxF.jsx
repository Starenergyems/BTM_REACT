...
//設定節點資訊
const nodeSetting = {
  auxmF: { id: `${nodeGroupSetting.auxF.id}:auxmF`, name: "AUXM-D" },
  fiberTerminal: {
    id: `${nodeGroupSetting.auxF.id}:fiberTerminal`,
    name: "光纖終端盒",
  },
  multiCircuitMeters: {
    id: `${nodeGroupSetting.auxF.id}:multiCircuitMeters`,
    name: "多迴路電表1\\00A0\\00A0\\00A0\\00A0\\00A0多迴路電表2\\00A0\\00A0\\00A0\\00A0\\00A0多迴路電表3",
  },
  rtuGw: { id: `${nodeGroupSetting.auxF.id}:rtuGw`, name: "RTU GW" },
  switch: { id: `${nodeGroupSetting.auxF.id}:switch`, name: "Switch" },
  ups: { id: `${nodeGroupSetting.auxF.id}:ups`, name: "UPS" },
};
//設定線路資訊
const edgeSetting = {
  "auxF:fiberTerminal-auxF:switch(1)": {
    id: `${nodeSetting.fiberTerminal.id}-to-${nodeSetting.switch.id}(1)`,
  },
  "auxF:fiberTerminal-auxF:switch(2)": {
    id: `${nodeSetting.fiberTerminal.id}-to-${nodeSetting.switch.id}(2)`,
  },
  "auxF:fiberTerminal-auxF:switch(3)": {
    id: `${nodeSetting.fiberTerminal.id}-to-${nodeSetting.switch.id}(3)`,
  },
  "auxF:fiberTerminal-auxF:switch(4)": {
    id: `${nodeSetting.fiberTerminal.id}-to-${nodeSetting.switch.id}(4)`,
  },
  "auxF:fiberTerminal-lc1000F1:fiberTerminal": {
    id: `${nodeSetting.fiberTerminal.id}-to-${indexConfigLc1000F1.nodeSetting.fiberTerminal.id}`,
  },
  "auxF:fiberTerminal-auxG:fiberTerminal": {
    id: `${nodeSetting.fiberTerminal.id}-to-${indexConfigAuxG.nodeSetting.fiberTerminal.id}`,
  },
  "auxF:fiberTerminal-lc1000G1:fiberTerminal": {
    id: `${nodeSetting.fiberTerminal.id}-to-${indexConfigLc1000G1.nodeSetting.fiberTerminal.id}`,
  },
  "auxF:switch-auxF:multiCircuitMeters": {
    id: `${nodeSetting.switch.id}-to-${nodeSetting.multiCircuitMeters.id}`,
  },
  "auxF:switch-auxF:rtuGw": {
    id: `${nodeSetting.switch.id}-to-${nodeSetting.rtuGw.id}`,
  },
  "auxF:rtuGw-auxF:ups": {
    id: `${nodeSetting.rtuGw.id}-to-${nodeSetting.ups.id}`,
  },
  "auxF:rtuGw-acpF1:trTemp": {
    id: `${nodeSetting.rtuGw.id}-to-${indexConfigAcpF1.nodeSetting.trTemp.id}`,
  },
  "auxF:ups-auxF:auxmF": {
    id: `${nodeSetting.ups.id}-to-${nodeSetting.auxmF.id}`,
  },
  "auxF:auxmF-acpF1:acpmF1-1": {
    id: `${nodeSetting.auxmF.id}-to-${indexConfigAcpF1.nodeSetting["acpmF1-1"].id}`,
  },
};
//節點初始值設定
const initialNodes = [
  //group：AUX-F
  {
    data: { label: nodeGroupSetting.auxF.name },
    id: nodeGroupSetting.auxF.id,
    position: { x: 670, y: 20500 },
    style: {
      ...nodeGroupStyle,
      width: nodeGap * 7 + 193,
  ...
  //auxF:光纖終端盒
  {
    ...
    parentNode: nodeGroupSetting.auxF.id,
    position: {
      x: startPositionX,
  ...
  //auxF:多迴路電表1 多迴路電表2 多迴路電表3
  {
    data: {
      label: (
        <NodeLabel
          label={nodeSetting.multiCircuitMeters.name}
          labelStyle={{ left: "180px", whiteSpace: "nowrap" }}
        >
          <Flex gap={16}>
...
const initialEdges = [
  {
    //auxF:光纖終端盒->Switch(1)
    id: edgeSetting["auxF:fiberTerminal-auxF:switch(1)"].id,
  ...
  {
    //auxF:光纖終端盒->Switch(2)
    id: edgeSetting["auxF:fiberTerminal-auxF:switch(2)"].id,
    source: nodeSetting.fiberTerminal.id,
  ...
  {
    //auxF:光纖終端盒->Switch(3)
    id: edgeSetting["auxF:fiberTerminal-auxF:switch(3)"].id,
    source: nodeSetting.fiberTerminal.id,
  ...
  {
    //auxF:光纖終端盒->Switch(4)
    id: edgeSetting["auxF:fiberTerminal-auxF:switch(4)"].id,
    source: nodeSetting.fiberTerminal.id,
    ...
    data: {
      ...
      pathValue(sourceX, sourceY, targetX, targetY) {
        return `
            ...
            L${sourceX - 115},${sourceY + nodeGap * 13 - 115}
            L${targetX},${targetY}
          `;
      },
    },
    id: edgeSetting["auxF:fiberTerminal-auxG:fiberTerminal"].id,
    source: nodeSetting.fiberTerminal.id,
    sourceHandle: "source-left-4", //index是表示node的設定中handles.source:{dir:left}是陣列第幾位
    style: { stroke: edgeColor.yellow, strokeWidth: 3 },
    target: indexConfigAuxG.nodeSetting.fiberTerminal.id,
    targetHandle: "target-left-0", //index是表示node的設定中handles.target:{dir:left}是陣列第幾位
    type: "custom",
  },
  {
    ...
    id: edgeSetting["auxF:fiberTerminal-lc1000G1:fiberTerminal"].id,
    source: nodeSetting.fiberTerminal.id,
    sourceHandle: "source-left-5", //index是表示node的設定中handles.source:{dir:left}是陣列第幾位
    style: { stroke: edgeColor.orange, strokeWidth: 3 },
    target: indexConfigLc1000G1.nodeSetting.fiberTerminal.id,
    targetHandle: "target-left-1", //index是表示node的設定中handles.target:{dir:left}是陣列第幾位
    type: "custom",
  },
  {
    //auxF:Switch->多迴路電表1 多迴路電表2 多迴路電表3
    data: {
      pathType: "custom",
      pathValue(sourceX, sourceY, targetX, targetY) {
        return `
    ...
    id: edgeSetting["auxF:switch-auxF:multiCircuitMeters"].id,
  ...
  {
    //auxF:Switch->多迴路電表1 多迴路電表2 多迴路電表2
    data: {
      pathType: "custom",
      pathValue(sourceX, sourceY, targetX, targetY) {
        return `
    ...
    id: edgeSetting["auxF:switch-auxF:rtuGw"].id,
    source: nodeSetting.switch.id,
    sourceHandle: "source-right-1", //index是表示node的設定中handles.source:{dir:right}是陣列第幾位
    style: { stroke: edgeColor.green, strokeWidth: 3 },
  ...
  },
  {
    ...
    id: edgeSetting["auxF:rtuGw-auxF:ups"].id,
  ...
  {
    //auxF:RTU GW->acpF1:TR Temp
    id: edgeSetting["auxF:rtuGw-acpF1:trTemp"].id,
    source: nodeSetting.rtuGw.id,
    sourceHandle: "source-right-0", //index是表示node的設定中handles.source:{dir:right}是陣列第幾位
    style: { stroke: edgeColor.red, strokeWidth: 3, strokeDasharray: "8 8" },
    target: indexConfigAcpF1.nodeSetting.trTemp.id,
    targetHandle: "target-left-0", //index是表示node的設定中handles.target:{dir:left}是陣列第幾位
    type: "custom",
  },
  {
    //auxF:UPS GW->AUXM-D
    id: edgeSetting["auxF:ups-auxF:auxmF"].id,
    source: nodeSetting.ups.id,
    sourceHandle: "source-right-0", //index是表示node的設定中handles.source:{dir:right}是陣列第幾位
    style: { stroke: edgeColor.red, strokeWidth: 3, strokeDasharray: "8 8" },
    target: nodeSetting.auxmF.id,
    targetHandle: "target-left-0", //index是表示node的設定中handles.target:{dir:left}是陣列第幾位
    type: "custom",
  },
  {
    //auxB:AUXM-D->acpF1:ACPM D1-1
    id: edgeSetting["auxF:auxmF-acpF1:acpmF1-1"].id,
    source: nodeSetting.auxmF.id,
    sourceHandle: "source-right-0", //index是表示node的設定中handles.source:{dir:right}是陣列第幾位
    style: { stroke: edgeColor.red, strokeWidth: 3, strokeDasharray: "8 8" },
    target: indexConfigAcpF1.nodeSetting["acpmF1-1"].id,
    targetHandle: "target-left-0", //index是表示node的設定中handles.target:{dir:left}是陣列第幾位
    type: "custom",
  },
];
