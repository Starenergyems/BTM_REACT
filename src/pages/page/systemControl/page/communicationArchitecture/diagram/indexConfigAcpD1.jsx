...
const nodeSetting = {
  "acpmD1-1": {
    id: `${nodeGroupSetting.acpD1.id}:acpmD1-1`,
    name: "ACPM D1-1",
  },
  "acpmD1-2": {
    id: `${nodeGroupSetting.acpD1.id}:acpmD1-2`,
    name: "ACPM D1-2",
  },
  remoteIo: { id: `${nodeGroupSetting.acpD1.id}:remoteIo`, name: "Remote I/O" },
  trTemp: { id: `${nodeGroupSetting.acpD1.id}:trTemp`, name: "TR Temp" },
};
//設定線路資訊
const edgeSetting = {
  "acpD1:trTemp-acpD2:trTemp": {
    id: `${nodeSetting.trTemp.id}-to-${indexConfigAcpD2.nodeSetting.trTemp.id}`,
  },
  "acpD1:acpmD1-1-acpD1:acpmD1-2": {
    id: `${nodeSetting["acpmD1-1"].id}-to-${nodeSetting["acpmD1-2"].id}`,
  },
  "acpD1:acpmD1-2-acpD2:acpmD2-1": {
    id: `${nodeSetting["acpmD1-2"].id}-to-${indexConfigAcpD2.nodeSetting["acpmD2-1"].id}`,
  },
  "acpD1:remoteIo-lc1000D1:switch": {
    id: `${nodeSetting.remoteIo.id}-to-${indexConfigLc1000D1.nodeSetting.switch.id}`,
  },
};
//節點初始值設定
const initialNodes = [
  //group：ACP-D1
  {
    data: { label: nodeGroupSetting.acpD1.name },
    id: nodeGroupSetting.acpD1.id,
    position: { x: 3425, y: 13900 },
    style: {
      ...nodeGroupStyle,
      width: nodeGap - 10,
  ...
  //acpD1:TR Temp
  {
    ...
    parentNode: nodeGroupSetting.acpD1.id,
  ...
  //acpD1:ACPM D1-1
  {
    data: {
      label: (
        <NodeLabel
          label={nodeSetting["acpmD1-1"].name}
  ...
  //acpD1:ACPM D1-2
  {
    data: {
      label: (
        <NodeLabel
          label={nodeSetting["acpmD1-2"].name}
          labelStyle={{ left: "50%", whiteSpace: "nowrap" }}
        >
          <div className="rect-icon bg-auxm"></div>
        </NodeLabel>
...
const initialEdges = [
  {
    //acpD1:TR Temp->acpD2:TR Temp
    id: edgeSetting["acpD1:trTemp-acpD2:trTemp"].id,
    ...
    target: indexConfigAcpD2.nodeSetting.trTemp.id,
    targetHandle: "target-left-0", //index是表示node的設定中handles.target:{dir:left}是陣列第幾位
    type: "custom",
  },
  {
    //acpD1:ACPM D1-1->ACPM D1-2
    id: edgeSetting["acpD1:acpmD1-1-acpD1:acpmD1-2"].id,
    source: nodeSetting["acpmD1-1"].id,
    sourceHandle: "source-right-0", //index是表示node的設定中handles.source:{dir:right}是陣列第幾位
    style: { stroke: edgeColor.red, strokeWidth: 3, strokeDasharray: "8 8" },
    target: nodeSetting["acpmD1-2"].id,
    targetHandle: "target-left-0", //index是表示node的設定中handles.target:{dir:left}是陣列第幾位
    type: "custom",
  },
  {
    //acpD1:ACPM D1-2->acpD2:ACPM D2-1
    id: edgeSetting["acpD1:acpmD1-2-acpD2:acpmD2-1"].id,
    source: nodeSetting["acpmD1-2"].id,
    sourceHandle: "source-right-0", //index是表示node的設定中handles.source:{dir:right}是陣列第幾位
    style: { stroke: edgeColor.red, strokeWidth: 3, strokeDasharray: "8 8" },
    target: indexConfigAcpD2.nodeSetting["acpmD2-1"].id,
    targetHandle: "target-left-0", //index是表示node的設定中handles.target:{dir:left}是陣列第幾位
    type: "custom",
  },
  {
    //acpD1:Remote I/O->lc1000D1:Switch
    data: {
      pathType: "custom",
    ...
    id: edgeSetting["acpD1:remoteIo-lc1000D1:switch"].id,
    ...
    target: indexConfigLc1000D1.nodeSetting.switch.id,
