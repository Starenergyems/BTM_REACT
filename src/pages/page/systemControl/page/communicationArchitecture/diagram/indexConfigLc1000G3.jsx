...
const nodeSetting = {
  fiberTerminal: {
    id: `${nodeGroupSetting.lc1000G3.id}:fiberTerminal`,
    name: "光纖終端盒",
  },
  lc: { id: `${nodeGroupSetting.lc1000G3.id}:lc`, name: "LC" },
  sgSwitch: {
    id: `${nodeGroupSetting.lc1000G3.id}:sgSwitch`,
    name: "SG_Switch",
  },
  switch: { id: `${nodeGroupSetting.lc1000G3.id}:switch`, name: "Switch" },
};
//設定線路資訊
const edgeSetting = {
  "lc1000G3:fiberTerminal-lc1000G3:switch(1)": {
    id: `${nodeSetting.fiberTerminal.id}-to-${nodeSetting.switch.id}(1)`,
  },
  "lc1000G3:fiberTerminal-lc1000G3:switch(2)": {
    id: `${nodeSetting.fiberTerminal.id}-to-${nodeSetting.switch.id}(2)`,
  },
  "lc1000G3:fiberTerminal-lc1000H1:fiberTerminal": {
    id: `${nodeSetting.fiberTerminal.id}-to-${indexConfigLc1000H1.nodeSetting.fiberTerminal.id}`,
  },
  "lc1000G3:switch-lc1000G3:lc": {
    id: `${nodeSetting.switch.id}-to-${nodeSetting.lc.id}`,
  },
  "lc1000G3:switch-lc1000G3:sgSwitch": {
    id: `${nodeSetting.switch.id}-to-${nodeSetting.sgSwitch.id}`,
  },
  "lc1000G3:lc-lc1000G3:sgSwitch": {
    id: `${nodeSetting.lc.id}-to-${nodeSetting.sgSwitch.id}`,
  },
  "lc1000G3:sgSwitch-noGroup:essG301Bsp": {
    id: `${nodeSetting.sgSwitch.id}-to-${indexConfigNoGroup.nodeSetting.essG301Bsp.id}`,
  },
};
//節點初始值設定
const initialNodes = [
  //group：LC1000-G3
  {
    data: { label: nodeGroupSetting.lc1000G3.name },
    id: nodeGroupSetting.lc1000G3.id,
    position: { x: 670, y: 26100 },
    style: {
      ...nodeGroupStyle,
      width: nodeGap * 5 - 75,
      height: nodeGap + 20,
  ...
  //lc1000G3:光纖終端盒
  {
    ...
    parentNode: nodeGroupSetting.lc1000G3.id,
...
const initialEdges = [
  {
    //lc1000G3:光纖終端盒->Switch(1)
    id: edgeSetting["lc1000G3:fiberTerminal-lc1000G3:switch(1)"].id,
  ...
  {
    //lc1000G3:光纖終端盒->Switch(2)
    id: edgeSetting["lc1000G3:fiberTerminal-lc1000G3:switch(2)"].id,
    source: nodeSetting.fiberTerminal.id,
  ...
  {
    //lc1000G3:光纖終端盒->lc1000H1:光纖終端盒
    id: edgeSetting["lc1000G3:fiberTerminal-lc1000H1:fiberTerminal"].id,
    source: nodeSetting.fiberTerminal.id,
    sourceHandle: "source-bottom-2", //index是表示node的設定中handles.source:{dir:bottom}是陣列第幾位
    style: { stroke: edgeColor.orange, strokeWidth: 3 },
    target: indexConfigLc1000H1.nodeSetting.fiberTerminal.id,
    targetHandle: "target-top-0", //index是表示node的設定中handles.target:{dir:top}是陣列第幾位
    type: "custom",
  },
  {
    //lc1000G3:Switch->Lc
    id: edgeSetting["lc1000G3:switch-lc1000G3:lc"].id,
  ...
  },
  {
    ...
    id: edgeSetting["lc1000G3:switch-lc1000G3:sgSwitch"].id,
    source: nodeSetting.switch.id,
  ...
  {
    //lc1000G3:LC->SG_Switch
    id: edgeSetting["lc1000G3:lc-lc1000G3:sgSwitch"].id,
  ...
  {
    //lc1000G3:SG_Switch->noGroup:ESS G3-01 BSP
    id: edgeSetting["lc1000G3:sgSwitch-noGroup:essG301Bsp"].id,
    source: nodeSetting.sgSwitch.id,
    sourceHandle: "source-right-0", //index是表示node的設定中handles.source:{dir:right}是陣列第幾位
    style: { stroke: edgeColor.green, strokeWidth: 3 },
    target: indexConfigNoGroup.nodeSetting.essG301Bsp.id,
    targetHandle: "target-left-0", //index是表示node的設定中handles.target:{dir:left}是陣列第幾位
    type: "custom",
  },
];
