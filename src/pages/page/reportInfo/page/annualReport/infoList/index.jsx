...
function InfoList() {
  const [mainState, setMainState] = useState({
    ...
    powerUsageTableData: [...Array(12)].map((_, index) => ({
      month: index + 1,
      chargeAmount: null,
      ...
      totalAuxiliaryPower: 87.4,
    })),
    serviceQualityTableData: [...Array(12)].map((_, index) => ({
      month: index + 1,
      awardedQuantity: 10,
      serviceQualityOne: 1,
        ...
        <Table
          ...
          summary={(pageData) => {
            ...
            return (
              <>
                <Table.Summary.Row>
                  <Table.Summary.Cell index={0}>全年統計</Table.Summary.Cell>
                ...
                <Table.Summary.Row>
                  <Table.Summary.Cell index={0}>去年統計</Table.Summary.Cell>
                  {totalArr.map((item, index) => {
                    return (
                      <Table.Summary.Cell
                        index={index + 1}
        ...
        <Table
          ...
          summary={(pageData) => {
            ...
            const totalArr = [
              ...
              { fieldName: "cycleCountMax", total: 0 },
              { fieldName: "cycleCountAvg", total: 0 },
              { fieldName: "cycleCountMin", total: 0 },
              { fieldName: "serviceDisruptionHour", total: 0 },
              { fieldName: "serviceDisruptionCapacity", total: 0 },
              { fieldName: "totalAuxiliaryPower", total: 0 },
            ];
            pageData.forEach((row, rowIndex) => {
              totalArr.forEach((item, index, arr) => {
                arr[index].total += Number(row[item.fieldName] || 0);
